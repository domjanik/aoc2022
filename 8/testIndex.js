...
function findLength(i, j, treeGrid) {
    ...
    const left = findLengthLeft(i, j, i, j, treeGrid, 0)
    const right = findLengthRight(i, j, i, j, treeGrid, 0)
    const top = findLengthTop(i, j, i, j, treeGrid, 0)
    const bottom = findLengthBottom(i, j, i, j, treeGrid, 0)

    const score =  left * right * top * bottom;

    if(score == 444528) {
    console.log(`[${i}][${j}]: ${top} * ${left} * ${bottom} * ${right} = ${score}`); }
    return score;
  }
